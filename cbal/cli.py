"""
Command-line interface for C-BAL.

Usage::

    cbal info
    cbal fit --data train.csv --prediction-length 14 --presets medium_quality --output my_predictor
    cbal predict --predictor my_predictor --data test.csv --output predictions.csv
    cbal leaderboard --predictor my_predictor
    cbal serve --predictor my_predictor --port 8000
"""
...
def main():
    parser = argparse.ArgumentParser(
        prog="cbal",
        description="C-BAL — AutoML Time Series Forecasting Library",
    )
    ...
    fit_p.add_argument("--output", "-o", default="cbal_predictor")
    ...
    if args.version:
        from cbal import __version__
        print(f"cbal {__version__}")
        sys.exit(0)
...
def _cmd_serve(args):
    from cbal.serving.app import run_server
